    ...
    async def lock(self, interaction: discord.Interaction["BallsDexBot"], button: Button):
        ...
        if self.bet.bettor1.locked and self.bet.bettor2.locked:
            await interaction.followup.send(
                "Your proposal has been locked. Now confirm again to start the FootballDex Bet.",
                ephemeral=True,
            )
        else:
            await interaction.followup.send(
...
class BetMenu:
    ...
    def _generate_embed(self):
        ...
        self.embed.title = f"{settings.plural_collectible_name.title()} FootballDex Bet betting"
    ...
    async def start(self):
        ...
        self.message = await self.channel.send(
            ...
            "is challenging you to a FootballDex Bet bet!",
            embed=self.embed,
    ...
    async def lock(self, bettor: BettingUser):
        ...
        if self.bettor1.locked and self.bettor2.locked:
            ...
            self.embed.title = f"{settings.plural_collectible_name.title()} FootballDex Bet - Confirmation"
    ...
    async def confirm(self, bettor: BettingUser) -> bool:
        """
        Confirm the bet and execute the FootballDex Bet if both users confirmed.
        """
        if bettor.accepted:
            return True
    ...
    async def _execute_coin_flip(self) -> bool:
        """
        Execute the FootballDex Bet and transfer balls to the winner.
        """
        try:
            ...
            # Perform the FootballDex Bet (50/50 chance)
            ...
            self.embed.title = f"{settings.plural_collectible_name.title()} FootballDex Bet - Results!"
            ...
            log.exception(
                f"Failed to execute FootballDex Bet: guild={self.message.guild.id} "  # type: ignore
                f"bettor1={self.bettor1.user.id} bettor2={self.bettor2.user.id}"
            )
