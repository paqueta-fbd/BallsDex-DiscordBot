...
# Command cooldown tracking - stores {user_id: last_command_time}
command_cooldowns = {}
...
class Claim(commands.GroupCog, name="packs"):
    ...
    async def daily(self, interaction: discord.Interaction[BallsDexBot]):
        ...
        # Check 5-second cooldown to prevent spam
        now = datetime.now(timezone.utc)
        if user_id in command_cooldowns:
            time_since_last = now - command_cooldowns[user_id]
            if time_since_last < timedelta(seconds=5):
                remaining = 5 - int(time_since_last.total_seconds())
                await interaction.response.send_message(
                    f"⏰ Please wait {remaining} more seconds before using this command again.",
                    ephemeral=True
                )
                return
        ...
        if log_channel:
            await log_channel.send(
                f"**{interaction.user.mention}** claimed a Daily pack and got **{ball.country}** (Use {3-new_remaining}/3)\n"
        ...
        logger.info(
            ...
            f"Daily use {3-new_remaining}/3 | Footballer ID: `#{ball.pk:0X}`"
        )
