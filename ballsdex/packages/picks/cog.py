...
# Ongoing pick tracking - stores {user_id: 'command_type'} 
ongoing_picks = {}
...
def check_ongoing_pick(user_id: int) -> tuple[bool, str | None]:
    """
    Check if user has an ongoing pick.
    Returns (has_ongoing_pick, pick_type)
    """
    if user_id in ongoing_picks:
        return True, ongoing_picks[user_id]
    return False, None


def set_ongoing_pick(user_id: int, pick_type: str):
    """Set an ongoing pick for a user"""
    ongoing_picks[user_id] = pick_type


def clear_ongoing_pick(user_id: int):
    """Clear the ongoing pick for a user"""
    if user_id in ongoing_picks:
        del ongoing_picks[user_id]
...
class Picks(commands.GroupCog, name="picks"):
    ...
    async def daily(self, interaction: discord.Interaction[BallsDexBot]):
        ...
        # Check for ongoing pick
        has_ongoing, ongoing_type = check_ongoing_pick(user_id)
        if has_ongoing:
            await interaction.response.send_message(
                f"⏰ You already have an ongoing {ongoing_type} pick! Please finish it before starting a new one.",
                ephemeral=True
            )
            return
        ...
        # Set ongoing pick before showing selection
        set_ongoing_pick(user_id, "daily")
        ...
        if view.selected_ball is None:
            ...
            # Clear ongoing pick on timeout
            clear_ongoing_pick(user_id)
            return
        ...
        # Clear ongoing pick when completed successfully
        clear_ongoing_pick(user_id)
    ...
    async def weekly(self, interaction: discord.Interaction[BallsDexBot]):
        ...
        # Set ongoing pick before showing selection
        set_ongoing_pick(user_id, "weekly")
